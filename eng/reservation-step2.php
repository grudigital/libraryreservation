    ...
    <section class="resume-section" id="about">
        <div class="resume-section-content">
            ...
            <form action="functions/reservation-step2.php" enctype="multipart/form-data" method="post" class="needs-validation"
                ...
                $sqltitulo1 = $conn->query("select english from languages where id = 27");
                $row1 = $sqltitulo1->fetch_row();

                $sqltitulo2 = $conn->query("select english from languages where id = 28");
                $row2 = $sqltitulo2->fetch_row();

                $sqltitulo3 = $conn->query("select english from languages where id = 29");
                $row3 = $sqltitulo3->fetch_row();

                $sqltitulo4 = $conn->query("select english from languages where id = 30");
                $row4 = $sqltitulo4->fetch_row();

                $sqltitulo5 = $conn->query("select english from languages where id = 31");
                $row5 = $sqltitulo5->fetch_row();

                $sqltitulo6 = $conn->query("select english from languages where id = 32");
                $row6 = $sqltitulo6->fetch_row();

                $sqltitulo7 = $conn->query("select english from languages where id = 33");
                $row7 = $sqltitulo7->fetch_row();

                $sqltitulo8 = $conn->query("select english from languages where id = 34");
                $row8 = $sqltitulo8->fetch_row();

                $sqltitulo9 = $conn->query("select english from languages where id = 35");
...
//                        echo "<label class='form-check-label' for='exampleRadios2'>Whole Day (09:00-17:00)</label>";
//                        echo "</div>";
